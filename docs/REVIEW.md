...
**Vite-Movie** ("Arch Stanton Movies") est une Progressive Web App (PWA) référençant un catalogue de films américains classiques et leurs acteurs. L'interface est en français.
...
Les pages utilisent une séparation claire entre logique (Container) et présentation (Component), visible notamment sur `Home/` :
...
Les collections Firestore (`movies`, `actors`) sont récupérées une seule fois puis mises en cache dans le `localStorage`. Les lectures suivantes utilisent directement le cache.
...
- **Composants réutilisables** bien abstraits (`SvgIcon`, `SemiCard`, `FullCard`)
...
**1. Conflit de fusion non résolu** — `src/components/molecules/Home/Home.component.tsx` (lignes 3–6)
...
Le fichier contient des marqueurs de conflit Git. L'import de `react-anime` est présent dans HEAD mais n'est jamais utilisé dans le composant.

---

**2. Clé API Firebase exposée** — `src/services/firebaseConfig.ts`

Les credentials Firebase (apiKey, authDomain, projectId…) sont écrits en dur dans le code source versionné. Ils devraient être externalisés dans des variables d'environnement :
...
**3. Bug dans la fusion des données de recherche** — `src/pages/Home/Home.container.tsx` (ligne ~35)
...
**4. Navigation dépréciée** — `src/pages/Actor/Actor.tsx` (ligne ~19) et `src/pages/Movies/Movies.tsx` (ligne ~21)
...
**5. Type `any[]` dans l'état** — `src/pages/Home/Home.container.tsx` (ligne ~14)
...
`JSON.parse` est appelé directement sans vérification préalable. Une donnée corrompue en cache ferait planter silencieusement l'application.
...
**7. Gestion d'erreur incohérente dans les services** — `src/services/services.ts`

Les blocs `catch` retournent l'objet `Error` au lieu de `null` ou `[]`, ce qui peut casser les contrats de type en aval.

**8. Absence d'Error Boundaries**

Aucun composant `ErrorBoundary` n'est défini. Une erreur dans le rendu d'une page crashe l'application entière sans message d'erreur utilisateur.
...
- Aliases de chemins configurés : `@molecules`, `@organisms`, `@pages`, `@assets`, `@services`, `@helpers`
- Plugin SVG avec symbol ID au format `icon-[dir]-[name]`
- PWA avec enregistrement automatique du service worker

### `vercel.json`

Réécriture SPA : toutes les routes renvoient vers `index.html` (nécessaire pour React Router).
...
1. **Résoudre le conflit de fusion** dans `Home.component.tsx` et supprimer l'import inutilisé de `react-anime`
2. **Externaliser les credentials Firebase** dans un fichier `.env` (et ajouter `.env` au `.gitignore`)
3. **Corriger le bug de spread** dans la construction de `searchData` (Home.container.tsx)
...
7. **Sécuriser les lectures localStorage** avec try/catch et invalidation du cache en cas d'erreur

### Priorité basse

8. **Étendre la couverture de tests** : pages, services (avec mock Firebase), composants
9. **Envisager une migration** vers React 18 et Vite 4+/5+ (versions actuelles datant de 2021–2022)
